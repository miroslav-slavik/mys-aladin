...
- **Místa (název, lat, lon):** jedno místo na soubor v adresáři `places/`,
  ...
  - `Černý Most` a `Nusle`, části Prahy, obec `Liběchov` na Mělnicku a obec
    `Hnojice` na Olomoucku. Souřadnice jsou středy z databáze Geonames ČÚZK,
    stejné jako v seznamu míst zabudovaném do aplikace. Nejbližší body mřížky
    leží 578 m, 438 m, 104 m a 446 m daleko.
